...
class Comment extends CI_Controller 
{
	...
	{		
	   ...
	   $to_author = $this->input->post('to_author', TRUE);
	   $comments = $this->comment_model->get_comment($note_id);
	   $note_owner = $data['session']['username'];
		$result = '';
		...
		foreach ($comments as $comm) {
			...
			$result = $result.'
					<div class="well comment" style="margin-bottom: 5px; padding: 5px; min-height: 50px;">
					  ...
					  <div class="comm_id" value="'.$comm['comment_id'].'" ></div>
		...
		if ( $reply_to != "" || $to_author != "") {
		    $this->load->model('notification_model');
		    $this->notification_model->insert_notification($comm_id, $reply_to, $to_author, $note_owner); //update notification
		}	   
		echo $result;
	}
